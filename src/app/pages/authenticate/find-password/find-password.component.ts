...
export default class FindPasswordComponent implements OnDestroy {
  ...
  submitHandler() {
    if (this.form.valid) {
      ...
      this.authService
        ...
        .subscribe({
          next: () => {
            this.translate.get(['tip', 'findPasswordSuccess']).subscribe((message) => {
              this.notification.success(message.tip, message.findPasswordSuccess, {
                nzDuration: 2000
              })
              setTimeout(() => {
